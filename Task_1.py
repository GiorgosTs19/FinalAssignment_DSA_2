        ...
        for i in range(size):
            ...
            for col in col_order:
                ...
                if is_safe(board, i, col):
                    ...
                    break
            if not row_success:
                # Trigger a failure, since this row hasn't been successful at placing a queen.
                failure = True
                break
    printBoard(board)
...
while not board_size.isnumeric() or int(board_size) <= 3:
    ...
    board_size = int(input("Enter a valid (number > 3) board size: "))
approach = input("Select an approach (1 -> Las Vegas, 2 -> Backtracking): ")
while not approach == "1" and not approach == "2":
    approach = int(input("Select a valid approach (1 -> Las Vegas, 2 -> Backtracking): "))

nQueensLasVegas(int(board_size))
